...
class DynamicDataset(BaseDataset):
    ...
    def build_validators(self, consent_sheet: str | None) -> List[ValidationResult]:
        ...
        cleaning_log_sheets = [item for item, value in self.sheet_matching.items() if value.log_type == 'cleaning']
        raw_sheets = [item for item, value in self.sheet_matching.items() if value.classification == 'raw']
        ...
        for sheet, details in self.sheet_matching.items():
            ...
            if details.classification == 'clean':
                ...
                if rowsum_sheets:
                    self.validators.append(CrossSheetRowSumCheck(schema=self.schema,
                                                            master_sheet=sheet,
        ...
        if consent_sheet is not None:
            ...
            results.append(ValidationResult(
                        ...
                        message = 'No possible sheet for consent was found.'
                        ,severity = SeverityLevel.WARNING
                    ))    
        
        if clean_sheets:# check unique
            self.validators.append(DataTypeCheck(schema=self.schema,
                                            check_sheets=clean_sheets))
            
            self.validators.append(SurveyChoicesCheck(schema=self.schema,
                                            check_sheets=clean_sheets))
            self.validators.append(NaNCheck(schema=self.schema,
                                            check_sheets=clean_sheets))
        else:
            results.append(ValidationResult(
                        rule = 'DynamicDataset Creation build_validators',
    ...
    def match_data(self) -> List[ValidationResult]:
        ...
        cleaning_log_sheets = [k for k, v in self.sheet_matching.items() if v.classification == "log" and v.log_type == 'cleaning']
        ...
        for log_name in cleaning_log_sheets:
            ...
            for data_name in data_sheets:
                ...
                if data_sheet.id_column in log_sheet.data.columns:
                    ...
                    if len(alt_matches) == 1:
                        ...
                        # or a common name
                        matching_columns = match_list(log_sheet.data.columns, settings.COMMON_ID_COLUMN_NAMES)
                ...
                if linking_log_column is not None:
                    ...
                    log_set = set(log_sheet.data.select(linking_log_column).to_series().unique().to_list())
            ...
            if best_score > min_matching_score and best_parent is not None:
                if log_sheet.log_type == "cleaning" and self.sheet_matching[best_parent].classification == "clean":
        ...
        unknown_sheets = [k for k, v in self.sheet_matching.items() if v.classification == 'unknown']
            ...
            for parent_name in sheets:
                ...
                if parent_prof.id_column is not None:
                    ...
                    if linking_column is not None:
                        child_set = set(child_prof.data.select(linking_column).to_series().unique().to_list())
