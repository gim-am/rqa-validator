...
class ExcelLoader:
    ...
    def load(self, filepath: Path, load_all_sheets: bool = False)   -> tuple[ExcelLoaderData, List[ValidationResult]]:
        ...
        for excel_sheet_name in all_sheets:
            ...
            l_mapped_name, l_results = match_excel_sheet_to_schema(excel_sheet_name, self.schema.schema_loaded_sheets)
            u_mapped_name, u_results = match_excel_sheet_to_schema(excel_sheet_name, self.schema.schema_unloaded_sheets)
            ...
            # 1 and 3
                ...
                if schema_sheet is not None:
                    ...
                    column_results, column_matches = match_excel_columns_to_schema(df_columns, schema_sheet)
            ...
            # 2, 4
                ...
                df_columns = df.columns
                df = df.rename(str.lower)
                data.loaded_sheets.append(DataSheetMap(schema_sheet_name=excel_sheet_name,
                                                            data_sheet_name = excel_sheet_name,
                                                            data=df,
                                                            data_columns=df_columns))    
