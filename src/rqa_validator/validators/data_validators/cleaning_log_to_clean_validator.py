...
class CleaningLogToClean(BaseValidator):
    ...
    def validate(self, data: ExcelLoaderData) -> List[ValidationResult]:
        ...
        multiple_change_df = modified_rows_df.filter(multiple_change_mask).sort(clean_log_id_columns.data_column_name)

        if multiple_change_df.height > 0:
            df_to_csv(data=multiple_change_df, filename = multiple_changes_filename)
            results.append(ValidationResult(
                rule = self.name,
                message = f'{multiple_change_df.select(clean_log_id_columns.data_column_name) \
        ...
        clean_data_filtered_df = data_loaded_sheets[self.clean_data_sheet].data.select([clean_data_id_columns.data_column_name] + questions) \
                                                                .filter(pl.col(clean_data_id_columns.data_column_name)
                                                                        .is_in(unique_modified_rows_df[clean_log_id_columns.data_column_name].implode())) \
                                                                # .fill_null('')
